...
const ServiceItem: FC<ServiceItemProps> = ({ icon, title, route }) => {
  return (
    <Link href={route}>
      <div
        className="relative pt-16 bg-gradient-to-b from-gray-100 to-gray-200 rounded-lg shadow-lg border-2 border-black transform transition-transform hover:scale-105 hover:shadow-2xl overflow-visible"
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full mt-5 text-gray-950">
          <h3 className="text-xl py-6 font-bold mb-2 underline decoration-[#94c2d2]">{title}</h3>
        </div>
      </div>
    </Link>
  );
